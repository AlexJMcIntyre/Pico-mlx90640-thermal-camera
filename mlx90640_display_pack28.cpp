...
#include "pico_display_2.hpp"  // Include Pico Display 2.8" header

using namespace pimoroni;

// Update for Pico Display 2.8"
...
// Display size for Pico Display 2.8"
const uint16_t WIDTH = 320;  // New width
const uint8_t HEIGHT = 240; // New height
...
int main() {
    ...
    // Calculate block size to scale the 32x24 sensor grid to the 320x240 display
    ...
    while (true) {
        ...
        graphics.text("Cold: " + format_float(coldest_temp) + "C", Point(10, HEIGHT - 20), 320, 2);  // Bottom left
        graphics.text("Hot: " + format_float(hottest_temp) + "C", Point(WIDTH - 100, HEIGHT - 20), 320, 2);  // Bottom right
